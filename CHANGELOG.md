...
- **`list` tool description + `ListParams.filter` / `AliasCreateParams.filter` / `AliasAggregateParams.filter` doc comments** (`crates/mcp/src/mcp/server.rs`) — filter primitive enumeration normalised to `Eq / In / Like / ArrayContains / ArrayNotContains / Or / And` (the previous wording omitted `Like`; fixed in the same commit for consistency).
